...
async function DashboardContent() {
  ...
  const averageWaterLevel =
    readingValues.length > 0
      ...
      : 0;
  const stationNames = new Map(snapshots.map((snapshot) => [snapshot.station.id, snapshot.station.name]));
  ...
  return (
    <>
      ...
      <section className="mb-8 max-w-3xl">
        <p className="mb-3 text-xs uppercase tracking-[0.18em] text-accent">Quan trắc trực tiếp</p>
        <h2 className="text-3xl font-semibold tracking-tight md:text-4xl">Tổng quan môi trường nước Cồn Hô</h2>
        <p className="mt-3 text-lg leading-relaxed text-muted">
          Trang này ưu tiên ba câu hỏi: độ mặn đang ở mức nào, mực nước đang thay đổi ra sao, và hệ thống có điểm nào
          cần chú ý ngay.
        </p>
      </section>

      <section aria-label="Tóm tắt vận hành" className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Độ mặn trung bình" value={formatSalinity(metrics.averageSalinity)} note="Dữ liệu mới nhất từ các trạm" />
        <MetricCard label="Mực nước trung bình" value={formatWaterLevel(averageWaterLevel)} note="Tổng hợp từ các bản ghi gần nhất" />
        <MetricCard label="Trạm hoạt động" value={`${metrics.activeStations}/${metrics.totalStations}`} note="Mức phủ của mạng lưới" />
        <MetricCard label="Cảnh báo cần chú ý" value={String(metrics.criticalAlerts)} note="Xuất hiện trong 24 giờ gần nhất" />
      </section>

      <section aria-label="Danh sách trạm" className="mb-8">
        <div className="mb-4">
          <h3 className="text-2xl font-semibold tracking-tight">Danh sách trạm</h3>
          <p className="mt-1 text-sm text-muted">Mỗi thẻ hiển thị chỉ số chính; chi tiết kỹ thuật nằm bên trong.</p>
        </div>
      ...
      <section aria-label="Cảnh báo" className="space-y-4">
        ...
        {allAlerts.length === 0 && (
          <p className="text-muted">Không có cảnh báo trong 7 ngày gần nhất. Mạng lưới đang vận hành bình thường.</p>
        )}
      </section>
    </>
  );
