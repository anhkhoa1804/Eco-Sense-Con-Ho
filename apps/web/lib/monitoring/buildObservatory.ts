...
import { qualityFor, stationProfiles } from "@/lib/stationProfile";
...
async function buildRealObservatory(): Promise<ObservatoryViewModel> {
  const context = getPublicRepositories();
  if (!context) return emptyRealObservatory();
  ...
  try {
    ...
    const stations: ObservatoryStation[] = PILOT_STATION_IDS.map((id) =>
      buildRealStation(id, snapshots, soilReading, alertStationIds.has(id)),
    );
    ...
    return {
      ...
      alerts: alerts.map(toObservatoryAlert),
      reference: buildReference(threshold),
    };
  } catch {
    return emptyRealObservatory();
  }
}

function emptyRealObservatory(): ObservatoryViewModel {
  const stations = PILOT_STATION_IDS.map((id) => buildRealStation(id, [], null, false));
  return {
    mode: "real",
...
function buildRealStation(
  ...
  if (profile.kind === "soil") {
    return {
      ...
      primary: metric("Độ ẩm đất", soilReading?.soil_moisture_pct, 1, "%", provenance),
      environment: [
        {
          label: "Đất",
          metrics: [
            metric("EC đất", soilReading?.soil_ec_ms_cm, 2, "mS/cm", provenance),
            metric("Độ pH", soilReading?.soil_ph, 1, undefined, provenance),
            metric("Nhiệt độ đất", soilReading?.soil_temp_c, 1, "°C", provenance),
          ],
        },
        {
          label: "Không khí",
          metrics: [
            metric("Nhiệt độ", soilReading?.air_temp_c, 1, "°C", provenance),
            metric("Độ ẩm", soilReading?.air_humidity_pct, 1, "%", provenance),
          ],
        },
      ],
      capabilityNote: null,
  ...
  if (profile.kind === "gateway") {
    ...
    return {
      ...
      primary: {
        ...
        unit: "dBm",
        provenance: signal !== null ? provenance : unavailable(GATEWAY_CAPABILITY_NOTE),
      },
      environment: [],
      capabilityNote: signal !== null ? null : GATEWAY_CAPABILITY_NOTE,
    };
  }
  ...
  return {
    ...
    primary: metric("Độ mặn", reading?.salinity, 2, "‰", provenance),
    environment: [
      {
        label: "Nước",
        metrics: [metric("Mực nước", reading?.water_level, 0, "cm", provenance)],
      },
    ],
    capabilityNote: null,
  };
...
function buildDemoObservatory(): ObservatoryViewModel {
  const stations = DEMO_STATION_SNAPSHOTS.map(buildDemoStation);
...
function buildDemoStation(snapshot: DemoStationSnapshot): ObservatoryStation {
  ...
  const device = [
    metric("Pin", snapshot.batteryVoltage, 2, "V", provenance),
    metric("Tín hiệu", snapshot.signalStrengthDbm, 0, "dBm", provenance),
  ];

  const base = {
    id: snapshot.id,
    name: snapshot.label,
    location: "Trạm minh họa — không gắn với vị trí thật",
  ...
  if (snapshot.kind === "soil") {
    return {
      ...
      kind: "soil",
      quality: "valid",
      primary: metric("Độ ẩm đất", snapshot.soilMoisturePct, 1, "%", provenance),
      environment: [
        {
          label: "Đất",
          metrics: [
            metric("EC đất", snapshot.soilEcMsCm, 2, "mS/cm", provenance),
            metric("Độ pH", snapshot.soilPh, 1, undefined, provenance),
            metric("Nhiệt độ đất", snapshot.soilTempC, 1, "°C", provenance),
          ],
        },
        {
          label: "Không khí",
          metrics: [
            metric("Nhiệt độ", snapshot.airTempC, 1, "°C", provenance),
            metric("Độ ẩm", snapshot.airHumidityPct, 1, "%", provenance),
          ],
        },
      ],
      device,
  ...
  if (snapshot.kind === "gateway") {
    return {
      ...
      kind: "gateway",
      quality: null,
      primary: metric("Tín hiệu", snapshot.signalStrengthDbm, 0, "dBm", provenance),
      environment: [],
      device: [metric("Pin", snapshot.batteryVoltage, 2, "V", provenance)],
      capabilityNote: null,
    };
  }
  ...
  return {
    ...
    kind: "water",
    quality: "valid",
    primary: metric("Độ mặn", snapshot.salinity, 2, "‰", provenance),
    environment: [
      { label: "Nước", metrics: [metric("Mực nước", snapshot.waterLevel, 0, "cm", provenance)] },
    ],
    device,
    capabilityNote: null,
  };
...
function toObservatoryAlert(alert: EnvironmentalEvent): ObservatoryAlert {
  return {
    id: alert.id,
    stationName: stationProfiles[alert.station_id]?.name ?? alert.station_id,
    severity: alert.severity,
    title: eventTitle(alert.event_type),
...
export async function getObservatoryViewModel(mode: "real" | "demo"): Promise<ObservatoryViewModel> {
  if (mode === "demo") return buildDemoObservatory();
  return buildRealObservatory();
}
